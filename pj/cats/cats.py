...
def time_per_word(words, times_per_player):
    ...
    for p in times_per_player:
        timel+=[[p[i]-p[i-1] for i in range(1,len(p))]]
    return match(words,timel)
    # END PROBLEM 9
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    wordl=get_words(match)
    timel=get_times(match)
    lowest_player_list=[]
    for windex in word_indices:
        lowest_time=time(match,0,windex)
        lowest_player_index=0
        for pindex in player_indices:
            timespend=time(match,pindex,windex)
            if timespend<lowest_time:
                lowest_time=timespend
                lowest_player_index=pindex
        lowest_player_list+=[lowest_player_index]
    print("DEBUG:lowestplist",lowest_player_list)
    rlist=[]
    for p in player_indices:
        rlist+=[[word_at(match,i) for i in range(len(lowest_player_list)) if lowest_player_list[i]==p]]
    return rlist
    # END PROBLEM 10
